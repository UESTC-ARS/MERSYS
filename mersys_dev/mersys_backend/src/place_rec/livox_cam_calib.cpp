...
// color_cloud1+=color_cloud;
  ...
  cv::Mat getProjectionImg(const Camera& camera_,pcl::PointCloud<pcl::PointXYZI>::Ptr& lidar_cloud_, const Vector6d& extrinsic_params)
  {
    ...
    auto Calibration::add_lidar(PointCloudEXPtr pc) -> pcl::PointCloud<pcl::PointXYZI>::Ptr
    {
        std::unordered_map<VOXEL_LOC, OCTO_TREE_ROOT*> surf_map;
        ...
        *cloud += pc->pts_cloud;
        std::cout << "edge extract size:"<< cloud->size() << std::endl;
        ne.setInputCloud(cloud);
        pcl::search::KdTree<pcl::PointXYZI>::Ptr tree(new pcl::search::KdTree<pcl::PointXYZI>());
        ne.setSearchMethod(tree);

        // 创建曲率计算对象
        pcl::PrincipalCurvaturesEstimation<pcl::PointXYZI, pcl::Normal, pcl::PrincipalCurvatures> pc_;
        pc_.setInputCloud(cloud);

        // 为法线估计和曲率计算设置搜索半径（可以根据需要调整）
        ne.setRadiusSearch(0.03); // 搜索半径为0.03
        pc_.setRadiusSearch(0.03);

        // 计算法线
        pcl::PointCloud<pcl::Normal>::Ptr normals(new pcl::PointCloud<pcl::Normal>);
        ne.compute(*normals);
        pc_.setInputNormals(normals);
        pc_.setSearchMethod(tree);
        // 计算曲率
        pcl::PointCloud<pcl::PrincipalCurvatures>::Ptr principal_curvatures(new pcl::PointCloud<pcl::PrincipalCurvatures>);
        pc_.compute(*principal_curvatures);

        // 边缘点云
        for (size_t i = 0; i < principal_curvatures->size(); ++i) {
        if (principal_curvatures->points[i].pc1 > 0.1 || principal_curvatures->points[i].pc2 > 0.1) {
            lidar_edge_cloud_->push_back(cloud->points[i]);
        }
    }
        ...
        return lidar_edge_cloud_;
    }
    auto Calibration::add_img(ImageEXPtr img,bool if_undistort=true) -> pcl::PointCloud<pcl::PointXYZ>::Ptr
    {
    ...
    auto Calibration::roughCalib(Camera& camera_,pcl::PointCloud<pcl::PointXYZI>::Ptr& lidar_edge_cloud_, pcl::PointCloud<pcl::PointXYZ>::Ptr& rgb_edge_cloud_, double search_resolution, int max_iter)  -> void
    {
      float match_dis = 25;
      ...
      for(int n = 0; n < 2; n++)
        for(int round = 0; round < 3; round++)
        {
          ...
          float min_cost = 1000;
          std::cout << COUTERROR<<"ROUCLIAB START:" << std::endl;
...
// ROS_INFO_STREAM("roughCalib1.2");
  ...
  { 
    ...
    {
      pcl::PointXYZI point = lidar_cloud[i];
      Eigen::Vector3d pt1(point.x, point.y, point.z);
      Eigen::Vector3d pt2(0, 0, 1);
      if(cos_angle(q_ * pt1 + t_, pt2) > cos(DEG2RAD(mersys_params::fusion::cam_fov_/2.0))) // FoV check
      {
